...
// SYCL-based Cuckaroo Cycle solver implementation
std::vector<uint32_t> c29s_gpu_sol_nonces(const uint64_t seed_k0, const uint64_t seed_k1,
  ...
  sycl::buffer<uint32_t, 1> buffer_recovery{sycl::range<1>{64}};

  // Buffer zeroing utility
  ...
  compute_queue.submit([&](sycl::handler& handler) {
    ...
    // Local memory for temporary storage and counters
    ...
    handler.parallel_for(sycl::nd_range<1>(sycl::range<1>(2048 * 128), sycl::range<1>(128)),
      ...
      // Process nonces in blocks to generate graph edges
  ...
  auto redistribute_edges = [](sycl::queue& queue, sycl::buffer<sycl::uint2, 1>& source_buffer,
                               ...
                               const uint32_t start_block, const uint32_t duck_edges_a) {
  ...
  redistribute_edges(compute_queue, buffer_a1_u2, buffer_a1_ul4, buffer_a2_ul4, buffer_i1, buffer_i2, 32, duck_edges_a);
  redistribute_edges(compute_queue, buffer_b_u2, buffer_a1_ul4, buffer_a2_ul4, buffer_i1, buffer_i2, 0, duck_edges_a);
  ...
  // Process each solution - recover nonces
  for (uint32_t sol_idx = 0; sol_idx < solutions.size(); sol_idx++) {
    ...
    compute_queue.submit([&](sycl::handler& handler) {
      ...
      handler.parallel_for(sycl::nd_range<1>(sycl::range<1>(2048 * 256), sycl::range<1>(256)),
                          [=](sycl::nd_item<1> item) {
        const uint32_t gid = item.get_global_id(0), lid = item.get_local_id(0);
    ...
    { sycl::host_accessor acc{buffer_nonces, sycl::read_only};
      std::copy(acc.get_pointer(), acc.get_pointer() + 32, nonces.begin());
    }

    // Sort nonces as required by Cuckaroo29 protocol
    std::sort(nonces.begin(), nonces.end());

    // Store recovered nonces in edge output buffer
    results.insert(results.end(), nonces.begin(), nonces.end());
  }
...
void c29s(const uint8_t* const inputs, const uint32_t input_size,
  ...
  try {
    ...
    // Generate SipHash keys from input using Blake2B
    ...
    // Solve Cuckaroo29 - returns vector of nonces (empty if no solution)
    const std::vector<uint32_t> solution_nonces = c29s_gpu_sol_nonces(k0, k1, k2, k3, q);

    // Check if solution was found
    const uint32_t num_solutions = solution_nonces.empty() ? 0 : solution_nonces.size() / C29_CYCLE_LEN;
    const uint32_t solutions_to_process = std::min(num_solutions, *pbatch);

    uint32_t* const edge_output = static_cast<uint32_t*>(output_edges);

    // Process each solution (each contains C29_CYCLE_LEN nonces)
    for (uint32_t sol_idx = 0; sol_idx < solutions_to_process; sol_idx++) {
      // Extract nonces for this solution
      std::vector<uint32_t> nonces(C29_CYCLE_LEN);
      std::copy(solution_nonces.begin() + sol_idx * C29_CYCLE_LEN,
                solution_nonces.begin() + (sol_idx + 1) * C29_CYCLE_LEN,
                nonces.begin());
      ...
      for (const uint32_t nonce : nonces) {
        for (uint32_t bit = 0; bit < EDGE_BITS; bit++) {
         if (nonce & (1u << bit)) {
             const uint32_t byte_idx = bit_pos / 8, bit_idx = bit_pos % 8;
      ...
      std::memcpy(edge_output + sol_idx * SPAD_LEN, nonces.data(), C29_CYCLE_LEN * sizeof(uint32_t));
    }
